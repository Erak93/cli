...
print('Welcome to the warehouse management system. Here is a list of the commands available:')
print()
...
while user_input!="4":
    ...
    if user_input == '2':
        ...
        item_name = input('What is the name of the item you are looking for? ')
        print()
    ...
    # if the sum of the len of the two list is equal to 0 (meaning that the for loops in wh1 and wh2 did not find anything) it means that there is no item in warehouse
        if len(wh1 + wh2) == 0:
            print("Sorry, that item in stock. Thank you for your visit!")
            break
        else:
    ...
    if user_input=='3':
        ...
        for i,(k,v) in enumerate(dictionary_group_category.items(),start=1):
            print(f'{i}. {k} ({v})')
        ...
        category_search_filter=category_list[browse_choice-1]
